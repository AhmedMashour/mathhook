...
const pow = (base, exp) => JsExpression.pow(base, exp);
const int = (n) => JsExpression.integer(n);
const float = (n) => JsExpression.float(n);
...
test("division expression", (t) => {
  const [x] = symbols("x");
  const expr = JsExpression.div(x, int(2));
...
test("complex number creation", (t) => {
  const z = JsExpression.complex(int(3), int(4));
