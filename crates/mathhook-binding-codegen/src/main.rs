    ...
    match cli.command {
        ...
        Commands::Generate {
            ...
            let emitter: Box<dyn Emitter> = if target_str == "python" {
                let mut emitter = PythonEmitter::with_simple_enum_types(simple_enum_types);
                if !no_stub_gen {
                    emitter.enable_stub_gen();
                    emitter.set_stub_gen_module("mathhook");
                    println!("  Stub generation: enabled");
                } else {
                    println!("  Stub generation: disabled");
                }
                Box::new(emitter)
            } else {
