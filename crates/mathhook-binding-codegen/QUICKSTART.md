# mathhook-binding-codegen Quick Start

**Last Updated:** 2025-12-27T1430

This guide covers the most common workflows for the mathhook-binding-codegen tool.

## First Time Setup

1. **Install the tool** (already done if in workspace):
   ```bash
   cargo build -p mathhook-binding-codegen
   ```

2. **Scan mathhook-core** to see what's available:
   ```bash
   cargo run -p mathhook-binding-codegen -- scan
   ```
...
cargo run -p mathhook-binding-codegen -- debug --type-name Expression

# Check Symbol bindings
cargo run -p mathhook-binding-codegen -- debug --type-name Symbol

# Inspect any other type
cargo run -p mathhook-binding-codegen -- debug --type-name YourType
```
...
```bash
cargo run -p mathhook-binding-codegen -- check
```

If stale, regenerate:
```bash
cargo run -p mathhook-binding-codegen -- generate
cargo run -p mathhook-binding-codegen -- generate --target node
```
...
```bash
cargo run -p mathhook-binding-codegen -- analyze
```
...
cargo run -p mathhook-binding-codegen -- debug --type-name ProblematicType
...
Configuration:
```
crates/mathhook-binding-codegen/binding-config.toml  # Override defaults (optional)
```
