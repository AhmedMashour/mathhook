//! Rust-based refactoring tool for mathhook-python
//!
...
#[command(about = "Refactor mathhook-python lib.rs into modules", long_about = None)]
struct Args {
    /// Path to lib.rs
...
impl Module {
    ...
    fn to_module_content(&self) -> String {
        ...
        content.push_str(&format!(
            "//! {} module for MathHook Python bindings\n",
...
impl Refactorer {
    ...
    fn categorize_function(&mut self, func: ItemFn, uses: &[syn::ItemUse]) -> Result<()> {
        // Check if function has #[pyfunction] attribute
        let has_pyfunction = func.attrs.iter().any(|attr| {
            attr.path().is_ident("pyfunction")
        });

        let module_name = if has_pyfunction {
            "functions"
        } else {
            "helpers"
        };
