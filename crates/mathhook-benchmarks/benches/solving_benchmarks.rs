...
fn bench_polynomial_solving(c: &mut Criterion) {
    ...
    for degree in [5, 7, 10].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("higher_degree", degree),
            degree,
            |b, _| b.iter(|| black_box(solver.solve(&poly, &x))),
        );
    }
...
fn bench_system_solving(c: &mut Criterion) {
    ...
    group.bench_function("2x2_linear_system", |b| {
        b.iter(|| black_box(solver.solve_system(&[eq1.clone(), eq2.clone()], &[x.clone(), y.clone()])))
    });
...
fn bench_matrix_equation_solving(c: &mut Criterion) {
    ...
    let A_2x2 = Expression::matrix(vec![
        vec![Expression::integer(1), Expression::integer(2)],
        vec![Expression::integer(3), Expression::integer(4)],
    ]);
    let B_2x2 = Expression::matrix(vec![
        vec![Expression::integer(5)],
        vec![Expression::integer(11)],
    ]);
    let eq_2x2 = Expression::add(vec![
        Expression::mul(vec![A_2x2, Expression::symbol(X.clone())]),
        Expression::mul(vec![Expression::integer(-1), B_2x2]),
    ]);

    group.bench_function("2x2_matrix_equation", |b| {
        b.iter(|| black_box(solver.solve(&eq_2x2, &X)))
    });

    // 3x3 matrix equation
    let A_3x3 = Expression::matrix(vec![
        vec![
            Expression::integer(2),
            Expression::integer(-1),
    ...
    let B_3x3 = Expression::matrix(vec![
        vec![Expression::integer(1)],
        vec![Expression::integer(2)],
        vec![Expression::integer(3)],
    ]);
    let eq_3x3 = Expression::add(vec![
        Expression::mul(vec![A_3x3, Expression::symbol(X.clone())]),
        Expression::mul(vec![Expression::integer(-1), B_3x3]),
    ]);

    group.bench_function("3x3_matrix_equation", |b| {
        b.iter(|| black_box(solver.solve(&eq_3x3, &X)))
    });
