...
use mathhook_core::{parse, symbol, Expression, Simplify, Symbol};
...
fn bench_derivatives(c: &mut Criterion) {
    ...
    group.bench_function("product_rule_with_parsing", |b| {
        b.iter(|| {
    ...
    group.bench_function("chain_rule_with_parsing", |b| {
        b.iter(|| {
...
fn bench_integrals(c: &mut Criterion) {
    ...
    group.bench_function("trigonometric_integral_sin_with_parsing", |b| {
        b.iter(|| {
            let expr = parse!("sin(x)").unwrap();
            let x = Symbol::new("x");
            let result = integrate_with_strategy(&expr, x, 0);
            black_box(result)
        })
    });
