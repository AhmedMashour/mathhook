...
fn bench_parsing_simple(c: &mut Criterion) {
    ...
    // Simple number
    group.bench_function("parse_number", |b| {
        b.iter(|| black_box(parser.parse("42")))
    });
    ...
    // Simple power
    group.bench_function("parse_power", |b| {
        b.iter(|| black_box(parser.parse("x^2")))
    });
...
fn bench_implicit_multiplication(c: &mut Criterion) {
    ...
    group.bench_function("parse_2x", |b| {
        b.iter(|| black_box(parser.parse("2x")))
    });
...
fn bench_parsing_throughput(c: &mut Criterion) {
    ...
    for size in [10, 20, 50, 100].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("sum_terms", size),
            size,
            |b, _| b.iter(|| black_box(parser.parse(&expr))),
        );
    }
