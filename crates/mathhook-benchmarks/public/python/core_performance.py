...
def bench_expression_creation_direct():
    ...
    x = Symbol("x")
    expr = Expression.add([Expression.symbol(x), Expression.integer(42)])
    return expr


def bench_expression_creation_with_parsing():
    """Benchmark expression creation (with parsing)."""
    expr = Expression.parse("x + 42")
    return expr
...
def bench_basic_solving_direct():
    ...
    solver = MathSolver()
    x = Symbol("x")
    equation = Expression.equation(Expression.symbol(x), Expression.integer(42))
...
def bench_basic_solving_with_parsing():
    ...
    solver = MathSolver()
    equation = Expression.parse("x = 42")
    x = Symbol("x")
    solutions = solver.solve(equation, x)
    return solutions
...
def bench_polynomial_creation_direct():
    ...
    terms = [
        Expression.multiply([
            ...
            Expression.pow(Expression.symbol(x), Expression.integer(10 - i))
        ])
        for i in range(11)
    ]
    poly = Expression.add(terms)
    return poly


def bench_polynomial_creation_with_parsing():
    """Benchmark polynomial creation (with parsing)."""
    poly = Expression.parse("x^10 + 2*x^9 + 3*x^8 + 4*x^7 + 5*x^6 + 6*x^5 + 7*x^4 + 8*x^3 + 9*x^2 + 10*x + 11")
    return poly
...
def bench_polynomial_simplification_with_parsing():
    ...
    poly = Expression.parse("x^2 - 5*x + 6")
    result = poly.simplify()
    return result
...
def bench_expression_size_verification():
    ...
    x = Symbol("x")
    expr = Expression.pow(Expression.symbol(x), Expression.integer(2))
    return expr
