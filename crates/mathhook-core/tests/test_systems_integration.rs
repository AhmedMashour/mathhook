...
/// 4. No critical regressions in routing (Fix 2: line 290 bug)
/// 5. Architecture follows CLAUDE.md patterns (registry-based, not hardcoded)
...
use mathhook_core::{expr, symbol, Expression, Symbol};
...
fn test_polynomial_system_detection() {
    ...
    assert!(
        matches!(equation_type, EquationType::Quadratic | EquationType::Cubic | EquationType::Quartic),
        "Should detect x² + y as polynomial equation, got {:?}", equation_type
    );
}
...
fn test_smart_solver_system_routing() {
    ...
    match result {
        ...
        _ => panic!("Expected unique solution for simple 2x2 linear system, got {:?}", result),
    }
}
...
fn test_polynomial_system_routes_to_groebner() {
    ...
    match result {
        ...
        _ => panic!("Expected Multiple or Partial for simple polynomial system, got {:?}", result),
    }
}
...
fn test_no_regression_linear_systems() {
    ...
    match result {
        ...
        _ => panic!("Expected unique solution for 3x3 linear system, got {:?}", result),
    }
}
...
fn test_architecture_no_hardcoded_routing() {
    ...
    // This is a meta-test ensuring CLAUDE.md architectural patterns are followed
