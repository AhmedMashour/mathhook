//! SymPy validation tests for derivative operations
//!
//! Tests validate MathHook derivatives against SymPy reference implementation
...
fn test_derivative_polynomial_cubic() {
    ...
    let x = symbol!(x);
    let expr = expr!(add: (x ^ 3), (3 * (x ^ 2)), (3 * x), 1);
    let result = expr.derivative(x.clone()).simplify();
    let expected = expr!(add: (3 * (x ^ 2)), (6 * x), 3);
    assert_eq!(result, expected);
}
...
fn test_derivative_sqrt() {
    ...
    let result = expr.derivative(x.clone());
    let expected = expr!(1 / (2 * (x ^ (1 / 2))));

    // Note: May need simplification comparison due to different canonical forms
