...
fn test_algebra_gcd_basic() {
    ...
    let expr2 = Expression::mul(vec![Expression::integer(6), Expression::symbol(x.clone())]);
...
fn test_algebra_gcd_performance_core() {
    ...
    for i in 1..2000 {
        let expr1 = Expression::mul(vec![Expression::integer(i), Expression::symbol(x.clone())]);
    ...
    println!(
        "Algebra GCD Core Performance: {:.2}K ops/sec",
        ops_per_sec / 1000.0
    );
