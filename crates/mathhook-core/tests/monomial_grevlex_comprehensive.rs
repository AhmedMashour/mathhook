...
fn test_grevlex_three_variables() {
    ...
    if cmp1 == Ordering::Less && cmp2 == Ordering::Less {
        assert_eq!(cmp3, Ordering::Less, "Transitivity: if a<b and b<c then a<c");
    } else if cmp1 == Ordering::Greater && cmp2 == Ordering::Greater {
        assert_eq!(cmp3, Ordering::Greater, "Transitivity: if a>b and b>c then a>c");
    }
}
...
fn test_grevlex_sorting() {
    ...
    let xz = Expression::mul(vec![Expression::symbol(x.clone()), Expression::symbol(z.clone())]);

    let mut monomials = vec![y2.clone(), xy.clone(), x2.clone(), z2.clone(), xz.clone()];
