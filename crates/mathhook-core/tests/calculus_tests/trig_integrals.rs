...
fn test_sin_cos_different_freq() {
    let _x = symbol!(x);
    // This test would require pattern detection for different frequencies
    // Currently not implemented in detect_trig_pattern
...
fn test_sin_sin_same_freq() {
    let x = symbol!(x);
    // sin(x)*sin(x) = sin²(x) already handled by sin_cos_power
...
fn test_cos_cos_same_freq() {
    let x = symbol!(x);
    // cos(x)*cos(x) = cos²(x) already handled by sin_cos_power
    let integrand = Expression::mul(vec![
        Expression::function("cos", vec![Expression::symbol(x.clone())]),
        Expression::function("cos", vec![Expression::symbol(x.clone())]),
    ]);
