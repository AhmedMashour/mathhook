...
fn test_multiplication_simplification() {
    ...
    // Test x * 0 = 0
    let expr = Expression::mul(vec![Expression::symbol(x.clone()), Expression::integer(0)]);
    let result = expr.simplify();
...
fn test_single_element_expressions() {
    ...
    let single_add = Expression::add(vec![Expression::symbol(x.clone())]).simplify();
    assert_eq!(single_add, Expression::symbol(x.clone()));

    let single_mul = Expression::mul(vec![Expression::symbol(x.clone())]).simplify();
...
fn test_performance_simplification() {
    ...
    println!(
        "Simplification performance: {:.2}K ops/sec",
        ops_per_sec / 1000.0
    );
...
fn test_rational_in_simplification() {
    ...
    let expr = Expression::add(vec![rational.clone(), rational.clone()]);
