...
use mathhook_core::{expr, Expression, MathError, Number, Simplify};
...
fn test_number_sign_helpers() {
    ...
    assert!(Expression::Number(Number::float(-3.6)).is_negative_number());

    // Test positive numbers
    assert!(Expression::integer(5).is_positive_number());
    assert!(Expression::rational(1, 2).is_positive_number());
    assert!(Expression::Number(Number::float(3.6)).is_positive_number());
    ...
    assert!(!expr!(x).is_negative_number());
    assert!(!expr!(x).is_positive_number());
}
