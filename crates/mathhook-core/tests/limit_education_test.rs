...
use mathhook_core::calculus::limits::{LimitEducation, Limits};
use mathhook_core::{symbol, Expression};

/// Helper function to check if any step contains the target text
fn has_step_containing(explanation: &mathhook_core::educational::enhanced_steps::EnhancedStepExplanation, target: &str) -> bool {
...
fn count_steps(explanation: &mathhook_core::educational::enhanced_steps::EnhancedStepExplanation) -> usize {
...
fn test_direct_substitution_explained() {
    ...
    assert!(count_steps(&explanation) >= 3, "Direct substitution should have at least 3 steps");

    assert!(
        has_step_containing(&explanation, "direct substitution") || has_step_containing(&explanation, "substitute"),
    ...
    assert!(
        has_step_containing(&explanation, "indeterminate") || has_step_containing(&explanation, "well-defined"),
...
fn test_indeterminate_form_components_evaluated() {
    ...
    assert!(has_step_containing(&explanation, "numerator") && has_step_containing(&explanation, "denominator"));
}
...
fn test_lhopital_rule_applied() {
    ...
    let explanation = LimitEducation::lhopital_rule_explanation(&numerator, &denominator, &x, &point);

    assert!(count_steps(&explanation) >= 6, "L'Hopital's rule should have at least 6 steps");

    assert!(
        has_step_containing(&explanation, "l'hopital") || has_step_containing(&explanation, "l'hospital"),
        "Should mention L'Hopital's rule"
    );

    assert!(
        has_step_containing(&explanation, "differentiate numerator") || has_step_containing(&explanation, "derivative of numerator"),
        "Should differentiate numerator"
    );

    assert!(
        has_step_containing(&explanation, "differentiate denominator") || has_step_containing(&explanation, "derivative of denominator"),
...
fn test_lhopital_rule_complete_process() {
    ...
    assert!(has_step_containing(&explanation, "state") || has_step_containing(&explanation, "rule"));
...
fn test_limit_laws_explained() {
    ...
    assert!(count_steps(&explanation) >= 4, "Limit laws should have at least 4 steps");
    ...
    assert!(
        has_step_containing(&explanation, "combine") || has_step_containing(&explanation, "individual"),
...
fn test_limit_at_infinity_polynomial() {
    ...
    assert!(has_step_containing(&explanation, "dominant") || has_step_containing(&explanation, "power"));
}
...
fn test_all_methods_produce_valid_json() {
    ...
    let direct_explanation = LimitEducation::direct_substitution_explanation(&expr, &x, &point, &result);
    assert!(direct_explanation.to_json().is_ok(), "Direct substitution should produce valid JSON");

    let indet_expr = Expression::mul(vec![
        Expression::symbol(x.clone()),
        Expression::pow(Expression::symbol(x.clone()), Expression::integer(-1))
    ]);
    let indet_explanation = LimitEducation::indeterminate_form_explanation(&indet_expr, &x, &Expression::integer(0), "0/0");
    assert!(indet_explanation.to_json().is_ok(), "Indeterminate form should produce valid JSON");

    let numerator = Expression::symbol(x.clone());
    let denominator = Expression::symbol(x.clone());
    let lhopital_explanation = LimitEducation::lhopital_rule_explanation(&numerator, &denominator, &x, &point);
    assert!(lhopital_explanation.to_json().is_ok(), "L'Hopital should produce valid JSON");

    let laws_explanation = LimitEducation::limit_laws_explanation(&expr, &x, &point);
    assert!(laws_explanation.to_json().is_ok(), "Limit laws should produce valid JSON");

    let infinity_explanation = LimitEducation::limit_at_infinity_explanation(&expr, &x);
    assert!(infinity_explanation.to_json().is_ok(), "Limit at infinity should produce valid JSON");
}
...
fn test_explanations_use_correct_variable_names() {
    ...
    let explanation = LimitEducation::direct_substitution_explanation(&expr, &theta, &point, &result);

    assert!(has_step_containing(&explanation, "theta"), "Should use correct variable name 'theta'");
}
...
fn test_step_ordering_logical() {
    ...
    assert!(steps[0].title.to_lowercase().contains("check") || steps[0].title.to_lowercase().contains("condition"));
    ...
    assert!(has_state_before_apply || steps[1].title.to_lowercase().contains("state"),
...
fn test_all_explanations_have_minimum_steps() {
    ...
    assert!(count_steps(&direct) >= 3, "Direct substitution needs 3+ steps");
