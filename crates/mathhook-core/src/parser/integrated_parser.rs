//! Fully integrated parser with implicit multiplication support
//!
//! This module provides the complete integration between:
//! 1. Enhanced lexer (detects implicit multiplication)
//! 2. LALRPOP grammar (conflict-free parsing)
//! 3. Expression AST (final result)

use crate::core::Expression;
use crate::parser::{
    lalrpop::grammar::mathematical, lexer_with_implicit_mul::ImplicitMultiplicationLexer,
    ParseError,
};

/// Fully integrated mathematical expression parser
pub struct IntegratedMathParser {
    enable_implicit_multiplication: bool,
}

impl IntegratedMathParser {
    ...
    /// Create parser without implicit multiplication
    ...
    ///
    /// ```rust
    /// use mathhook_core::parser::integrated_parser::IntegratedMathParser;
    ///
    ...
    fn tokens_to_string(
        &self,
        tokens: &[(usize, crate::parser::lalrpop::lexer::tokens::Token, usize)],
    ) -> String {
    ...
    fn token_to_string<'a>(
        &self,
        token: &crate::parser::lalrpop::lexer::tokens::Token<'a>,
    ) -> &'a str {
...
impl Default for IntegratedMathParser {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Expression {
    ...
    /// let expr = Expression::parse_with_implicit_mul("2x").unwrap();      // -> 2 * x
    /// let expr = Expression::parse_with_implicit_mul("xy").unwrap();      // -> x * y
    /// let expr = Expression::parse_with_implicit_mul("2(x+1)").unwrap();  // -> 2 * (x + 1)
    ///
    /// // Regular cases:
    /// let expr = Expression::parse_with_implicit_mul("x + y").unwrap();   // -> x + y
    /// let expr = Expression::parse_with_implicit_mul("x^2").unwrap();     // -> x^2
    /// ```
    pub fn parse_with_implicit_mul(input: &str) -> Result<Expression, ParseError> {
        let parser = IntegratedMathParser::new();
        parser.parse(input)
    }

    /// Parse with explicit operators only (no implicit multiplication)
    pub fn parse_explicit(input: &str) -> Result<Expression, ParseError> {
        let parser = IntegratedMathParser::explicit_only();
        parser.parse(input)
    }
}
...
mod tests {
    ...
    fn test_parser_modes() {
        let implicit_parser = IntegratedMathParser::new();
        let explicit_parser = IntegratedMathParser::explicit_only();
    ...
    #[test]
    fn test_expression_convenience_methods() {
        // Test the convenient Expression methods
        let result = Expression::parse_explicit("2 * x");
        assert!(result.is_ok());

        // This will work once implicit multiplication is fully integrated
        // let result = Expression::parse_smart("2x");
        // assert!(result.is_ok());
    }
}
