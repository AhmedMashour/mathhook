...
use crate::ode::first_order::linear::LinearFirstOrderSolver;
...
impl BernoulliODESolver {
    ...
    ///
    /// ```rust
    /// use mathhook_core::ode::first_order::bernoulli::BernoulliODESolver;
    ...
    pub fn solve(
        ...
        if n_simplified == Expression::integer(0) || n_simplified == Expression::integer(1) {
            return Err(ODEError::NotLinearForm {
                reason: "Bernoulli equation requires n ≠ 0, 1 (this is linear)".to_string(),
            });
        }
...
mod tests {
    ...
    fn test_bernoulli_solution_structure() {
        ...
        let solver = BernoulliODESolver;
        let solution = solver.solve(&expr!(1), &expr!(1), &expr!(2), &y, &x).unwrap();
