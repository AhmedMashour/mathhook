    ...
    F: Fn(f64, f64) -> f64,
{
    ...
    let mut x = x0;
    let mut y = y0;
    let mut h = initial_step.min(config.max_step);
    ...
    F: Fn(f64, f64) -> f64,
{
    if initial_step <= 0.0 {
        return Err(ODEError::InvalidInput {
            message: "Initial step size must be positive".to_string(),
        });
    }
...
mod tests {
    ...
    fn test_adaptive_better_accuracy() {
        use crate::ode::numerical::runge_kutta::rk4_method;
