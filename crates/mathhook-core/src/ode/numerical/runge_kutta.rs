...
///
/// ```
/// use mathhook_core::ode::numerical::runge_kutta::rk4_method;
///
...
mod tests {
    ...
    fn test_rk4_trigonometric() {
        ...
        let (x_final, y_final) = solution.last().unwrap();
        assert!((x_final - std::f64::consts::PI).abs() < 1e-10);
    ...
    fn test_rk4_variable_step() {
        let solution1 = rk4_method(|x, _y| x, 0.0, 0.0, 1.0, 0.1);
        let solution2 = rk4_method(|x, _y| x, 0.0, 0.0, 1.0, 0.05);
    ...
    fn test_rk4_better_than_euler() {
        use crate::ode::numerical::euler::euler_method;
