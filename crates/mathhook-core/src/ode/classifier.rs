...
impl ODEClassifier {
    ...
    ///
    /// ```rust
    /// use mathhook_core::ode::classifier::{ODEClassifier, ODEType};
        ...
        match rhs {
            Expression::Add(terms) => {
                ...
                for term in terms.iter() {
                    if Self::contains_symbol(term, dependent) {
                        if Self::is_linear_in_y(term, dependent) {
                            has_y_term = true;
                        } else {
                            return false;
                        }
                    } else if Self::contains_symbol(term, independent) {
                        has_const_term = true;
                    }
                }
            ...
            _ => !Self::contains_symbol(rhs, dependent) || Self::is_linear_in_y(rhs, dependent)
        }
    }
    ...
    fn is_linear_in_y(expr: &Expression, y: &Symbol) -> bool {
        match expr {
            ...
            Expression::Mul(factors) => {
                let mut y_count = 0;
                for factor in factors.iter() {
                    if Self::contains_symbol(factor, y) {
    ...
    fn has_y_power(expr: &Expression, y: &Symbol) -> bool {
        match expr {
            ...
            Expression::Mul(factors) => {
                factors.iter().any(|f| Self::has_y_power(f, y))
            }
            _ => false
        }
    }
    ...
    fn contains_symbol(expr: &Expression, symbol: &Symbol) -> bool {
        match expr {
...
mod tests {
    ...
    fn test_is_linear_in_y_symbol() {
        let y = symbol!(y);
        assert!(ODEClassifier::is_linear_in_y(&Expression::symbol(y.clone()), &y));
    }
