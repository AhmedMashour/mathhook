    ...
    pub fn is_constant(&self) -> bool {
        match self {
            ...
            Expression::Add(terms) | Expression::Mul(terms) => {
                terms.iter().all(|t| t.is_constant())
            }
            Expression::Pow(base, exp) => base.is_constant() && exp.is_constant(),
            Expression::Function { args, .. } => args.iter().all(|a| a.is_constant()),
            ...
            Expression::MethodCall(method_data) => {
                method_data.object.is_constant() && method_data.args.iter().all(|a| a.is_constant())
            }
        }
    }
