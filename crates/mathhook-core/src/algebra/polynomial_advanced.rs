...
impl AdvancedPolynomial for Expression {
    ...
    fn polynomial_leading_coefficient(&self, var: &Symbol) -> Expression {
        match self {
            ...
            Expression::Mul(factors) => {
                ...
                for factor in factors.iter() {
                    if factor.polynomial_degree(var).is_some()
                        && !matches!(factor, Expression::Symbol(s) if s == var)
    ...
    fn polynomial_primitive_part(&self) -> Expression {
        ...
        if content.is_zero() || content == Expression::integer(1) {
            ...
            Expression::mul(vec![
                self.clone(),
                Expression::pow(content, Expression::integer(-1)),
            ])
        }
    }
    ...
    fn polynomial_resultant(&self, other: &Self, var: &Symbol) -> Expression {
        ...
        Expression::function(
            "resultant",
            vec![self.clone(), other.clone(), Expression::symbol(var.clone())],
        )
    }
    ...
    fn polynomial_discriminant(&self, var: &Symbol) -> Expression {
        ...
        Expression::function(
            "discriminant",
            vec![self.clone(), Expression::symbol(var.clone())],
        )
    }
}
...
impl PolynomialArithmetic {
    ...
    pub fn evaluate_polynomial(poly: &Expression, var: &Symbol, value: &Expression) -> Expression {
        ...
        match poly {
            ...
            Expression::Add(terms) => {
                let evaluated_terms: Vec<Expression> = terms
                    .iter()
            ...
            Expression::Mul(factors) => {
                let evaluated_factors: Vec<Expression> = factors
                    .iter()
...
mod tests {
    ...
    fn test_polynomial_arithmetic_performance() {
        ...
        for _ in 0..1000 {
            ...
            let _evaluation =
                PolynomialArithmetic::evaluate_polynomial(&poly1, &x, &Expression::integer(5));
        }
        ...
        println!(
            "Advanced polynomial ops: {:.2}K ops/sec",
            ops_per_sec / 1_000.0
        );
