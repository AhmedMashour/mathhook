...
impl PolynomialSolver {
    ...
    /// Static helper function for recursive degree calculation.
    /// Does not require instance state, only performs expression traversal.
            ...
            if terms.len() == 2 {
                ...
                if let Expression::Pow(base, exp) = power_term {
                        ...
                        && **exp == Expression::integer(3)
                    {
                        ...
                        if cube_root_value.fract().abs() < EPSILON {
                ...
                if let Expression::Pow(base, exp) = power_term {
                        ...
                        && **exp == Expression::integer(4)
                    {
                        ...
                        if fourth_root_value.is_finite() {
                            ...
                            let real_root_expr = if real_root.fract().abs() < EPSILON {
                                Expression::integer(real_root as i64)
                            } else {
                                Expression::Number(Number::float(real_root))
                            };

                            let neg_real_root_expr = if real_root.fract().abs() < EPSILON {
                                Expression::integer(-(real_root as i64))
                            } else {
                                Expression::Number(Number::float(-real_root))
                            };
    ...
    ///
    /// Static helper function for recursive substitution.
    /// Does not require instance state, only performs expression traversal and replacement.
