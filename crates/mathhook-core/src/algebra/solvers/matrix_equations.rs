...
impl MatrixEquationSolver {
                ...
                match (&terms[0], &terms[1]) {
                    (Expression::Mul(factors), b) if factors.len() == 2 => {
                        if let [a, Expression::Symbol(x)] = &factors[..] {
                            if x == variable && !self.contains_variable(a, variable) {
                                // Found A*X - B pattern
                                let neg_b = Expression::mul(vec![
    ...
    fn contains_variable(&self, expr: &Expression, variable: &Symbol) -> bool {
        match expr {
            Expression::Symbol(s) => s == variable,
            Expression::Add(terms) | Expression::Mul(terms) => {
                terms.iter().any(|t| self.contains_variable(t, variable))
            }
            Expression::Pow(base, exp) => {
                self.contains_variable(base, variable)
                    || self.contains_variable(exp, variable)
            }
            Expression::Function { args, .. } => {
                args.iter().any(|a| self.contains_variable(a, variable))
            }
            _ => false,
        }
    }
    ...
    pub fn solve_left_division(
        ...
        if self.is_zero_matrix(a) {
            return Err(SolverError::InvalidEquation(
                "Cannot invert zero matrix".to_string(),
            ));
        }
    ...
    fn variable_appears_multiple_times(
        &self,
        expr: &Expression,
        variable: &Symbol,
    ) -> bool {
        let count = self.count_variable_occurrences(expr, variable);
        count > 1
    }

    /// Count variable occurrences in expression
    fn count_variable_occurrences(&self, expr: &Expression, variable: &Symbol) -> usize {
        match expr {
            Expression::Symbol(s) if s == variable => 1,
            Expression::Add(terms) | Expression::Mul(terms) => terms
                .iter()
                .map(|t| self.count_variable_occurrences(t, variable))
                .sum(),
            Expression::Pow(base, exp) => {
                self.count_variable_occurrences(base, variable)
                    + self.count_variable_occurrences(exp, variable)
            }
            Expression::Function { args, .. } => args
                .iter()
                .map(|a| self.count_variable_occurrences(a, variable))
                .sum(),
            _ => 0,
        }
    }
...
impl EquationSolver for MatrixEquationSolver {
    ...
    fn solve_with_explanation(
        ...
        // Check commutativity
        let is_commutative = self.check_commutativity(equation);
        if is_commutative {
            steps.push(Step::new(
                "Analysis",
        ...
        if let Some((a, b)) = self.detect_left_division(equation, variable) {
            steps.push(Step::new(
                "Pattern",
                format!("Identified left division: {} * {} = {}", a, variable.name, b),
            ));
            steps.push(Step::new(
                "Solution Method",
                format!(
            ...
            match self.solve_left_division(&a, &b) {
                Ok(solution) => {
                    steps.push(Step::new("Result", format!("{} = {}", variable.name, solution)));
                    return (
        ...
        if let Some((a, b)) = self.detect_right_division(equation, variable) {
            steps.push(Step::new(
                "Pattern",
                format!("Identified right division: {} * {} = {}", variable.name, a, b),
            ));
            steps.push(Step::new(
                "Solution Method",
                format!(
    ...
    fn can_solve(&self, equation: &Expression) -> bool {
        !self.check_commutativity(equation)
    }
}
