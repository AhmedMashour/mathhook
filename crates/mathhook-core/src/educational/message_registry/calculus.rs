//! Calculus educational messages for derivatives, integrals, limits, and summations
...
pub fn initialize_calculus_messages(registry: &mut HashMap<MessageKey, MessageTemplate>) {
    ...
    initialize_summation_messages(registry);
}
...
/// Initialize summation and series educational messages
fn initialize_summation_messages(registry: &mut HashMap<MessageKey, MessageTemplate>) {
    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationIntroduction, 0),
        MessageTemplate::new(
            "Summation Introduction",
            "We need to compute the sum Σ{variable} from {start} to {end}\nThis is a finite series with {term_count} terms",
            &["variable", "start", "end", "term_count"]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationIntroduction, 1),
        MessageTemplate::new(
            "Infinite Series Introduction",
            "Computing infinite sum Σ{expression} from {start} to ∞\nWe must first check if the series converges",
            &["expression", "start"]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationArithmeticSeries, 0),
        MessageTemplate::new(
            "Arithmetic Series Detected",
            "This is an arithmetic series with:\nFirst term a = {first_term}\nCommon difference d = {common_difference}\nNumber of terms n = {term_count}",
            &["first_term", "common_difference", "term_count"]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationArithmeticSeries, 1),
        MessageTemplate::new(
            "Arithmetic Series Formula",
            "For an arithmetic series, we use the formula:\nSum = n/2 × (2a + (n-1)d)\nWhere n is the number of terms, a is the first term, and d is the common difference",
            &[]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationGeometricSeries, 0),
        MessageTemplate::new(
            "Geometric Series Detected",
            "This is a geometric series with:\nFirst term a = {first_term}\nCommon ratio r = {common_ratio}\nNumber of terms n = {term_count}",
            &["first_term", "common_ratio", "term_count"]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationGeometricSeries, 1),
        MessageTemplate::new(
            "Geometric Series Formula",
            "For a finite geometric series when r ≠ 1:\nSum = a(1 - r^n) / (1 - r)\nFor infinite series with |r| < 1:\nSum = a / (1 - r)",
            &[]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationGeometricSeries, 2),
        MessageTemplate::new(
    ...
    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationPowerSum, 0),
        MessageTemplate::new(
            "Power Sum: Sum of Constants",
            "Sum of constants: Σ1 from 1 to n = n\nThis is the simplest case where each term is 1",
            &[],
        ),
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationPowerSum, 1),
        MessageTemplate::new(
            "Power Sum: Sum of Integers",
            "Sum of integers: Σi from 1 to n = n(n+1)/2\nThis is Gauss's famous formula for summing consecutive integers",
            &[]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationPowerSum, 2),
        MessageTemplate::new(
            "Power Sum: Sum of Squares",
            "Sum of squares: Σi² from 1 to n = n(n+1)(2n+1)/6\nThis formula sums the squares of the first n positive integers",
            &[]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationPowerSum, 3),
        MessageTemplate::new(
            "Power Sum: Sum of Cubes",
            "Sum of cubes: Σi³ from 1 to n = [n(n+1)/2]²\nRemarkably, this equals the square of the sum of integers (Nicomachus's theorem)",
            &[]
        )
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationPowerSum, 4),
        MessageTemplate::new(
            "Power Sum: Higher Powers",
            "Sum of {power}-th powers uses Faulhaber's formula\nFor higher powers, these formulas become increasingly complex polynomials in n",
            &["power"]
        )
    );
    ...
    registry.insert(
        ...
        MessageTemplate::new(
            "Divergence Detected",
            "Series diverges because {reason}\nDivergent series do not have a finite sum",
            &["reason"],
        ),
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationFormula, 0),
        MessageTemplate::new(
            "Apply Formula",
            "Substituting into {formula_name}:\n{substitution_details}",
            &["formula_name", "substitution_details"],
        ),
    );
    ...
    registry.insert(
        MessageKey::new(
            MessageCategory::Calculus,
            MessageType::SummationSubstitution,
            0,
        ),
        MessageTemplate::new(
            "Substitute Values",
            "Substituting n = {n_value}, a = {a_value}, {additional_params}:\n{calculation}",
            &["n_value", "a_value", "additional_params", "calculation"],
        ),
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationResult, 0),
        MessageTemplate::new(
            "Final Result",
            "The sum evaluates to: {result}\nThis is the exact value of the series",
            &["result"],
        ),
    );

    registry.insert(
        MessageKey::new(MessageCategory::Calculus, MessageType::SummationResult, 1),
        MessageTemplate::new(
            "Simplified Result",
            "Simplifying the sum:\n{steps}\nFinal answer: {result}",
            &["steps", "result"],
