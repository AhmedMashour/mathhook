...
mod tests {
    ...
    fn test_message_count() {
        ...
        assert!(
            count >= 95,
            "Expected at least 95 messages (including ODE), found {}",
            count
        );
    }
    ...
    fn test_ode_messages_exist() {
        ...
        assert!(ode_msg.is_some());
    }

    #[test]
    fn test_pde_messages_exist() {
        use crate::educational::message_registry::pde::{
            pde_message_key, PdeMessageVariant, PDE_MESSAGES,
        };

        let heat_key = pde_message_key(MessageType::Introduction, PdeMessageVariant::HEAT_EQUATION);
        let message = PDE_MESSAGES.get(&heat_key);

        assert!(message.is_some());
        assert!(message.unwrap().contains("Heat Equation"));
    }
}
