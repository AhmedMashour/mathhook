...
impl Default for PresentationHints {
    fn default() -> Self {
        Self {
            ...
            importance: 3,
            animation: "fade-in".to_owned(),
            layout: "standard".to_owned(),
...
impl EnhancedStepExplanation {
    ...
    pub fn to_api_data(&self) -> HashMap<String, serde_json::Value> {
        ...
        let step_data: Vec<serde_json::Value> = self
            ...
            .collect();
        data.insert("steps".to_owned(), serde_json::Value::Array(step_data));
    ...
    fn determine_method(steps: &[super::EnhancedStep]) -> String {
        ...
        {
            "Quadratic Formula".to_owned()
        } else if steps
            .iter()
            .any(|s| s.api_data.operation.contains("isolation"))
        {
            "Variable Isolation".to_owned()
        } else {
            "General Algebraic Method".to_owned()
        }
    }
    ...
    fn determine_prerequisites(steps: &[super::EnhancedStep]) -> Vec<String> {
        let mut prereqs = vec!["Basic Algebra".to_owned()];

        if steps
            .iter()
            .any(|s| s.api_data.category == "quadratic_equation")
        {
            prereqs.push("Quadratic Equations".to_owned());
        }
    ...
    fn extract_answer(steps: &[super::EnhancedStep]) -> String {
        if let Some(last_step) = steps.last() {
            last_step
                ...
                .unwrap_or(&"Solution in progress".to_owned())
                .clone()
        } else {
    ...
    fn suggest_next_steps(_steps: &[super::EnhancedStep]) -> Vec<String> {
        vec![
            "Try solving similar equations".to_owned(),
            "Practice with different coefficients".to_owned(),
            "Explore quadratic equations".to_owned(),
        ]
    }
}
