...
impl SpecialIntelligence {
    ...
    pub fn get_all_properties(&self) -> Vec<(String, FunctionProperties)> {
        vec![
            ("gamma".to_owned(), Self::gamma_properties()),
            ("beta".to_owned(), Self::beta_properties()),
            ("digamma".to_owned(), Self::digamma_properties()),
            ("polygamma".to_owned(), Self::polygamma_properties()),
            ("bessel_j".to_owned(), Self::bessel_j_properties()),
            ("bessel_y".to_owned(), Self::bessel_y_properties()),
            ("zeta".to_owned(), Self::zeta_properties()),
        ]
    }
    ...
    fn gamma_properties() -> FunctionProperties {
        FunctionProperties::Special(Box::new(SpecialProperties {
            ...
            recurrence_relations: vec![RecurrenceRule {
                name: "Functional equation".to_owned(),
                relation: r"\Gamma(z+1) = z \cdot \Gamma(z)".to_owned(),
                coefficients: vec![],
            }],
            ...
            special_values: vec![
                SpecialValue {
                    ...
                    latex_explanation: r"\Gamma(1) = 1".to_owned(),
                },
                SpecialValue {
                    input: "2".to_owned(),
                    output: Expression::integer(1),
                    latex_explanation: r"\Gamma(2) = 1".to_owned(),
                },
                SpecialValue {
                    input: "1/2".to_owned(),
                    output: Expression::mul(vec![Expression::sqrt(Expression::pi())]),
                    latex_explanation: r"\Gamma(1/2) = \sqrt{\pi}".to_owned(),
                },
                SpecialValue {
                    input: "3/2".to_owned(),
                    ...
                    latex_explanation: r"\Gamma(3/2) = \frac{\sqrt{\pi}}{2}".to_owned(),
                },
                SpecialValue {
                    input: "5/2".to_owned(),
                    output: Expression::div(
                    ...
                    latex_explanation: r"\Gamma(5/2) = \frac{3\sqrt{\pi}}{4}".to_owned(),
                },
            ],
            asymptotic_behavior: Some(AsymptoticData {
                as_x_to_infinity:
                    r"\Gamma(z) \sim \sqrt{2\pi/z} (z/e)^z \text{ (Stirling's approximation)}"
                        .to_owned(),
                as_x_to_zero: r"\Gamma(z) \sim 1/z \text{ for } z \to 0^+".to_owned(),
                leading_coefficient: Expression::integer(1),
            }),
            wolfram_name: Some("Gamma"),
        }))
    }
    ...
    fn beta_properties() -> FunctionProperties {
        FunctionProperties::Special(Box::new(SpecialProperties {
            ...
            antiderivative_rule: None,
            recurrence_relations: vec![RecurrenceRule {
                name: "Symmetry".to_owned(),
                relation: r"B(a,b) = B(b,a)".to_owned(),
                coefficients: vec![],
            }],
            differential_equation: None,
            special_values: vec![SpecialValue {
                input: "1, 1".to_owned(),
                output: Expression::integer(1),
                latex_explanation: r"B(1,1) = 1".to_owned(),
            }],
            asymptotic_behavior: None,
            wolfram_name: Some("Beta"),
        }))
    }
    ...
    fn digamma_properties() -> FunctionProperties {
        FunctionProperties::Special(Box::new(SpecialProperties {
            ...
            antiderivative_rule: None,
            recurrence_relations: vec![RecurrenceRule {
                name: "Recurrence relation".to_owned(),
                relation: r"\psi(z+1) = \psi(z) + \frac{1}{z}".to_owned(),
                coefficients: vec![],
            }],
            differential_equation: None,
            special_values: vec![],
            asymptotic_behavior: None,
            wolfram_name: None, // No direct Wolfram equivalent
        }))
    }
    ...
    fn bessel_j_properties() -> FunctionProperties {
        FunctionProperties::Special(Box::new(SpecialProperties {
            ...
            recurrence_relations: vec![
                RecurrenceRule {
                    name: "Sum relation".to_owned(),
                    relation: r"J_{n-1}(z) + J_{n+1}(z) = \frac{2n}{z}J_n(z)".to_owned(),
                    coefficients: vec![],
                },
                RecurrenceRule {
                    name: "Difference relation".to_owned(),
                    relation: r"J_{n-1}(z) - J_{n+1}(z) = 2J'_n(z)".to_owned(),
                    coefficients: vec![],
                },
                RecurrenceRule {
                    name: "Reflection formula".to_owned(),
                    relation: r"J_{-n}(z) = (-1)^n J_n(z)".to_owned(),
                    coefficients: vec![],
                },
            ],
            differential_equation: Some(DifferentialEquation {
                order: 2,
                equation: r"z^2 y'' + z y' + (z^2 - n^2)y = 0".to_owned(),
                coefficients: vec![],
            }),
            special_values: vec![
                SpecialValue {
                    input: "0, 0".to_owned(),
                    output: Expression::integer(1),
                    latex_explanation: r"J_0(0) = 1".to_owned(),
                },
                SpecialValue {
                    input: "n, 0".to_owned(),
                    output: Expression::integer(0),
                    latex_explanation: r"J_n(0) = 0 \text{ for } n > 0".to_owned(),
                },
            ],
            asymptotic_behavior: Some(AsymptoticData {
                as_x_to_infinity: r"J_n(z) \sim \sqrt{\frac{2}{\pi z}} \cos\left(z - \frac{n\pi}{2} - \frac{\pi}{4}\right)".to_owned(),
                as_x_to_zero: r"J_0(0) = 1, J_n(0) = 0 \text{ for } n > 0".to_owned(),
                leading_coefficient: Expression::integer(1),
            }),
            wolfram_name: None,  // Wolfram uses BesselJ[n, x] with different syntax
        }))
    }
    ...
    fn bessel_y_properties() -> FunctionProperties {
        FunctionProperties::Special(Box::new(SpecialProperties {
            ...
            recurrence_relations: vec![
                RecurrenceRule {
                    name: "Sum relation".to_owned(),
                    relation: r"Y_{n-1}(z) + Y_{n+1}(z) = \frac{2n}{z}Y_n(z)".to_owned(),
                    coefficients: vec![],
                },
                RecurrenceRule {
                    name: "Difference relation".to_owned(),
                    relation: r"Y_{n-1}(z) - Y_{n+1}(z) = 2Y'_n(z)".to_owned(),
                    coefficients: vec![],
                },
                RecurrenceRule {
                    name: "Reflection formula".to_owned(),
                    relation: r"Y_{-n}(z) = (-1)^n Y_n(z)".to_owned(),
                    coefficients: vec![],
                },
            ],
            differential_equation: Some(DifferentialEquation {
            ...
            special_values: vec![SpecialValue {
                input: "n, 0".to_owned(),
                output: Expression::integer(-1),
                latex_explanation: r"Y_n(0) = -\infty \text{ (logarithmic singularity)}".to_owned(),
            }],
            asymptotic_behavior: Some(AsymptoticData {
                as_x_to_infinity: r"Y_n(z) \sim \sqrt{\frac{2}{\pi z}} \sin\left(z - \frac{n\pi}{2} - \frac{\pi}{4}\right)".to_owned(),
                as_x_to_zero: r"Y_n(0) = -\infty \text{ (logarithmic singularity)}".to_owned(),
                leading_coefficient: Expression::integer(1),
            }),
            wolfram_name: None,  // Wolfram uses BesselY[n, x] with different syntax
        }))
    }
    ...
    fn zeta_properties() -> FunctionProperties {
        FunctionProperties::Special(Box::new(SpecialProperties {
            ...
            recurrence_relations: vec![RecurrenceRule {
                name: "Functional equation".to_owned(),
                relation: r"\zeta(s) = 2^s \pi^{s-1} \sin\left(\frac{\pi s}{2}\right) \Gamma(1-s) \zeta(1-s)".to_owned(),
                coefficients: vec![],
            }],
            differential_equation: None,
            special_values: vec![
                SpecialValue {
                    ...
                    latex_explanation: r"\zeta(0) = -\frac{1}{2}".to_owned(),
                },
                SpecialValue {
                    input: "-1".to_owned(),
                    output: Expression::rational(-1, 12),
                    latex_explanation: r"\zeta(-1) = -\frac{1}{12}".to_owned(),
                },
                SpecialValue {
                    input: "2".to_owned(),
                    output: Expression::div(
                    ...
                    latex_explanation: r"\zeta(2) = \frac{\pi^2}{6}".to_owned(),
                },
                SpecialValue {
                    input: "4".to_owned(),
                    output: Expression::div(
                    ...
                    latex_explanation: r"\zeta(4) = \frac{\pi^4}{90}".to_owned(),
                },
                SpecialValue {
                    input: "6".to_owned(),
                    output: Expression::div(
                    ...
                    latex_explanation: r"\zeta(6) = \frac{\pi^6}{945}".to_owned(),
                },
                SpecialValue {
                    input: "8".to_owned(),
                    output: Expression::div(
                    ...
                    latex_explanation: r"\zeta(8) = \frac{\pi^8}{9450}".to_owned(),
                },
                SpecialValue {
                    input: "10".to_owned(),
                    output: Expression::div(
                    ...
                    latex_explanation: r"\zeta(10) = \frac{\pi^{10}}{93555}".to_owned(),
                },
                SpecialValue {
                    input: "-5".to_owned(),
                    output: Expression::rational(-1, 252),
                    latex_explanation: r"\zeta(-5) = -\frac{1}{252}".to_owned(),
                },
                SpecialValue {
                    input: "-7".to_owned(),
                    output: Expression::rational(1, 240),
                    latex_explanation: r"\zeta(-7) = \frac{1}{240}".to_owned(),
                },
            ],
            asymptotic_behavior: Some(AsymptoticData {
                as_x_to_infinity: r"\zeta(s) \to 1 \text{ as } \text{Re}(s) \to \infty".to_owned(),
                as_x_to_zero: r"\zeta(s) \text{ has pole at } s=1 \text{ with residue } 1".to_owned(),
                leading_coefficient: Expression::integer(1),
            }),
            wolfram_name: Some("Zeta"),
        }))
    }
}
...
impl StepGenerator for SpecialStepGenerator {
    ...
    fn generate_latex_explanation(&self, name: &str, args: &[Expression]) -> String {
        match name {
            "gamma" => format!(
                r"\Gamma({})",
                args.first().map(|e| e.to_string()).unwrap_or_default()
            ),
            "beta" => format!(
                r"B({}, {})",
                args.first().map(|e| e.to_string()).unwrap_or_default(),
...
impl SpecialStepGenerator {
    ...
    fn bessel_j_steps(&self, args: &[Expression]) -> Vec<Step> {
        ...
        steps.push(Step::new(
            ...
            "Bessel J functions solve: z²y'' + zy' + (z² - n²)y = 0".to_owned(),
        ));
    ...
    fn bessel_y_steps(&self, args: &[Expression]) -> Vec<Step> {
        ...
        steps.push(Step::new(
            "Mathematical Context",
            "Bessel Y functions solve: z²y'' + zy' + (z² - n²)y = 0".to_owned(),
        ));

        steps.push(Step::new(
            "Warning",
            "Y functions have logarithmic singularity at z = 0".to_owned(),
        ));
    ...
    fn zeta_steps(&self, args: &[Expression]) -> Vec<Step> {
        ...
        steps.push(Step::new(
            "Definition",
            "For Re(s) > 1: ζ(s) = Σ(n=1 to ∞) 1/n^s".to_owned(),
        ));

        steps.push(Step::new(
            "Special Values",
            "The Riemann zeta function has known values: ζ(2)=π²/6, ζ(4)=π⁴/90, ζ(0)=-1/2, ζ(-1)=-1/12".to_owned(),
        ));

        steps.push(Step::new(
            "Pole at s=1",
            "Note: ζ(s) has a simple pole at s=1 with residue 1".to_owned(),
        ));
