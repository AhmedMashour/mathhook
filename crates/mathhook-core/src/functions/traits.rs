...
/// Trait for function evaluation capabilities
///
/// Separates evaluation concerns from intelligence concerns,
/// allowing different evaluation strategies for different function types.
pub trait FunctionEvaluator: Send + Sync {
    /// Evaluate a function symbolically
    fn evaluate_symbolic(&self, name: &str, args: &[Expression]) -> Option<Expression>;

    /// Evaluate a function numerically
    fn evaluate_numeric(&self, name: &str, args: &[f64]) -> Option<f64>;

    /// Check if this evaluator can handle a specific function
    fn can_evaluate(&self, name: &str) -> bool;

    /// Get evaluation accuracy for this function
    fn evaluation_accuracy(&self, name: &str) -> Option<f64>;
}
