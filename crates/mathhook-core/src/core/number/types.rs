//! Core number type and basic operations
//!
//! Defines the Number enum supporting integers, rationals, floats, and big integers.
//! Provides constructors, basic trait implementations, and helper methods.
...
impl Number {
    ...
    pub fn is_one(&self) -> bool {
        match self {
            ...
            Number::Float(f) => (f - 1.0).abs() < EPSILON,
    ...
    pub(crate) fn to_float(&self) -> Result<f64, MathError> {
        match self {
            Number::Integer(i) => Ok(*i as f64),
            Number::Float(f) => Ok(*f),
            Number::BigInteger(bi) => bi.to_f64().ok_or_else(|| MathError::NumericOverflow {
...
mod size_tests {
    ...
    #[test]
    fn test_number_size_constraint() {
        assert_eq!(
            std::mem::size_of::<Number>(),
            16,
            "Number size constraint violated! Expected 16 bytes, got {} bytes. \
             This breaks memory layout guarantees.",
            std::mem::size_of::<Number>()
        );
    }
}
