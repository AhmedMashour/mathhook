...
pub fn explain_constant_derivative(expr: &Expression, variable: &Symbol) -> StepByStepExplanation {
    ...
    steps.push(Step {
        title: "Identify Constant".to_owned(),
        description: format!(
        ...
        latex: Some(expr.to_latex(None).unwrap_or_else(|_| "c".to_owned())),
    });
    ...
    steps.push(Step {
        ...
        rule_applied: "Constant Rule".to_owned(),
        latex: Some("0".to_owned()),
    });
    ...
    StepByStepExplanation {
        ...
        rules_used: vec!["Constant Rule".to_owned()],
    }
}
...
pub fn explain_variable_derivative(sym: &Symbol, variable: &Symbol) -> StepByStepExplanation {
    ...
    if sym == variable {
        ...
        steps.push(Step {
            ...
            expression: result.clone(),
            rule_applied: "Variable Rule".to_owned(),
            latex: Some("1".to_owned()),
        });
        ...
        StepByStepExplanation {
            ...
            steps,
            total_steps: step_count,
            rules_used: vec!["Variable Rule".to_owned()],
        }
    } else {
        steps.push(Step {
            title: "Different Variable".to_owned(),
            description: format!(
...
pub fn explain_sum_derivative(terms: &[Expression], variable: &Symbol) -> StepByStepExplanation {
    ...
    steps.push(Step {
        title: "Identify Sum".to_owned(),
        description: format!("Function is a sum of {} terms", terms.len()),
        expression: expr.clone(),
        rule_applied: "Identification".to_owned(),
        latex: Some(expr.to_latex(None).unwrap_or_else(|_| "sum".to_owned())),
    });

    steps.push(Step {
        title: "Apply Sum Rule".to_owned(),
        description:
            "d/dx[f + g + h + ...] = f' + g' + h' + ...\nDifferentiate each term separately"
                .to_owned(),
        expression: expr.clone(),
        rule_applied: "Sum Rule".to_owned(),
        latex: Some(expr.to_latex(None).unwrap_or_else(|_| "sum".to_owned())),
    });
    ...
    for (i, term) in terms.iter().enumerate() {
        ...
        steps.push(Step {
            ...
            latex: Some(
                term_derivative
                    .to_latex(None)
                    .unwrap_or_else(|_| "term".to_owned()),
            ),
        });
    }
    ...
    steps.push(Step {
        title: "Combine Results".to_owned(),
        description: format!("Add all derivatives: {}", format_expr(&result)),
        expression: result.clone(),
        rule_applied: "Combination".to_owned(),
        latex: Some(
            result
                .to_latex(None)
    ...
    StepByStepExplanation {
        ...
        steps,
        total_steps: step_count,
        rules_used: vec!["Sum Rule".to_owned()],
    }
}
...
pub fn explain_power_rule(
    ...
    steps.push(Step {
        title: "Identify Power Function".to_owned(),
        description: format!(
            "Function is a power: {}^{}",
            format_expr(base),
            format_expr(exp)
        ...
        expression: expr.clone(),
        rule_applied: "Identification".to_owned(),
        latex: Some(expr.to_latex(None).unwrap_or_else(|_| "x^n".to_owned())),
    });
    ...
    steps.push(Step {
        title: "Apply Power Rule".to_owned(),
        description: "d/dx(x^n) = n*x^(n-1)".to_owned(),
        expression: expr.clone(),
        rule_applied: "Power Rule".to_owned(),
        latex: Some("n \\cdot x^{n-1}".to_owned()),
    });
    ...
    steps.push(Step {
        ...
        expression: result.clone(),
        rule_applied: "Simplification".to_owned(),
        latex: Some(
            result
                .to_latex(None)
                .unwrap_or_else(|_| "result".to_owned()),
    ...
    StepByStepExplanation {
        ...
        steps,
        total_steps: step_count,
        rules_used: vec!["Power Rule".to_owned()],
    }
}
