...
impl HigherOrderDerivatives {
    ...
    pub fn find_pattern(expr: &Expression, variable: Symbol, check_orders: u32) -> Option<u32> {
        ...
        (1..=(check_orders / 2)).find(|&period| Self::has_period(&derivatives, period as usize))
    }
    ...
    /// Get derivative of standard function using UniversalFunctionRegistry
    ///
    /// Uses registry-based derivative lookup for all registered functions.
    /// Falls back to symbolic derivative for unknown functions.
    fn get_function_derivative(function_name: &str, arg: &Expression) -> Expression {
        ...
        if let Some(props) = registry.get_properties(function_name) {
            if let Some(derivative) = props.get_derivative_expression(arg) {
                return derivative;
            }
        }
        Expression::derivative(
...
mod tests {
    ...
    fn test_trigonometric_patterns() {
        ...
        let sin_table = HigherOrderDerivatives::derivative_table(&sin_x, &x, 8);
        assert_eq!(sin_table.len(), 9);
        let cos_table = HigherOrderDerivatives::derivative_table(&cos_x, &x, 8);
    ...
    fn test_leibniz_rule() {
        ...
        let u = Expression::symbol(x.clone());
        let v = Expression::function("sin", vec![Expression::symbol(x.clone())]);
        let leibniz_2 = HigherOrderDerivatives::leibniz_rule(&u, &v, &x, 2);
        assert!(!leibniz_2.is_zero());
        let leibniz_3 = HigherOrderDerivatives::leibniz_rule(&u, &v, &x, 3);
        ...
        let leibniz_poly = HigherOrderDerivatives::leibniz_rule(&u_poly, &v_exp, &x, 2);
