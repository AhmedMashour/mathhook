...
impl PartialDerivatives {
    ...
    pub fn mixed_partial(expr: &Expression, variables: Vec<Symbol>) -> Expression {
        // Pre-allocate to avoid repeated cloning
        let mut result = expr.clone();

        // Apply derivatives in sequence - more memory efficient than recursion
    ...
    pub fn jacobian(functions: &[Expression], variables: Vec<Symbol>) -> Vec<Vec<Expression>> {
        JacobianOperations::compute(functions, &variables)
    }

    /// Compute Jacobian determinant - delegates to specialized module
    pub fn jacobian_determinant(functions: &[Expression], variables: Vec<Symbol>) -> Expression {
