...
impl ConservativeFields {
    ...
    fn is_conservative_3d(vector_field: &[Expression], variables: Vec<Symbol>) -> bool {
        ...
        curl.iter().all(PartialUtils::is_zero)
    }
    ...
    /// ];
    /// let potential = ConservativeFields::find_potential(&conservative_field, &[x, y]);
    /// ```
    pub fn find_potential(vector_field: &[Expression], variables: &[Symbol]) -> Option<Expression> {
        if !Self::is_conservative(vector_field, variables.to_vec()) {
            return None;
        }
        match vector_field.len() {
