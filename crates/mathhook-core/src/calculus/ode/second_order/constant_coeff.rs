...
/// Configuration for second-order constant coefficient ODE
#[derive(Debug, Clone)]
pub struct SecondOrderODEConfig<'a> {
    /// Coefficient of y''
    pub a: &'a Expression,
    /// Coefficient of y'
    pub b: &'a Expression,
    ...
    pub independent: &'a Symbol,
    /// Initial conditions (y(x0), y'(x0))
    pub initial_conditions: Option<(Expression, Expression)>,
}
...
impl ConstantCoeffSecondOrderSolver {
    ...
    /// # Complexity
    ///
    /// * **Time:** O(1) for characteristic equation solving (quadratic formula)
    /// * **Space:** O(1) for storing roots and solution components
    ///
    /// # Arguments
    ///
    ...
    ///
    /// ```rust
    /// use mathhook_core::calculus::ode::second_order::ConstantCoeffSecondOrderSolver;
    ...
    pub fn solve(
        ...
        let config = SecondOrderODEConfig {
            a,
            b,
            c,
            r,
    ...
    /// Solve using configuration struct
    pub fn solve_with_config(&self, config: &SecondOrderODEConfig) -> ODEResult {
        // Handle degenerate case: a = 0 (actually first-order)
        if config.a.is_zero() {
            return Err(ODEError::NotLinearForm {
                reason: "Coefficient 'a' is zero - this is a first-order ODE, not second-order"
                    .to_owned(),
            });
        }

        // Solve characteristic equation: ar² + br + c = 0
        let roots = self.solve_characteristic(config.a, config.b, config.c)?;

        // Form homogeneous solution based on root type
        let y_h = self.homogeneous_solution(&roots, config.independent)?;

        // Handle non-homogeneous case
        let _solution = if config.r.is_zero() {
            // Homogeneous: y = y_h
            y_h
        } else {
            // Non-homogeneous: y = y_h + y_p
            return Err(ODEError::NotImplemented {
                feature: "non-homogeneous constant coefficient ODEs".to_owned(),
            });
        };
    ...
    fn solve_characteristic(
        ...
        let discriminant = Expression::add(vec![
            ...
            Expression::mul(vec![Expression::integer(-4), a.clone(), c.clone()]),
        ])
        .simplify();
    ...
    fn classify_by_discriminant(
        ...
        if let Expression::Number(num) = discriminant {
            ...
            Err(ODEError::NotImplemented {
                feature: "symbolic discriminant classification".to_owned(),
            })
        }
    }

    /// Form homogeneous solution based on root type
    fn homogeneous_solution(&self, roots: &RootType, independent: &Symbol) -> ODEResult {
        let x = Expression::symbol(independent.clone());
        let constants = symbols![C1, C2];
        let c1 = Expression::symbol(constants[0].clone());
        let c2 = Expression::symbol(constants[1].clone());
        ...
        match roots {
            ...
            RootType::ComplexConjugate { alpha, beta } => {
                ...
                let exp_term = Expression::function(
                    "exp",
                    vec![Expression::mul(vec![alpha.clone(), x.clone()])],
                );
                let cos_term = Expression::mul(vec![
                    c1,
                    Expression::function(
                        "cos",
                        vec![Expression::mul(vec![beta.clone(), x.clone()])],
                    ),
                ]);
                let sin_term = Expression::mul(vec![
                    c2,
                    Expression::function("sin", vec![Expression::mul(vec![beta.clone(), x])]),
                ]);

                Ok(Expression::mul(vec![
                    exp_term,
                    Expression::add(vec![cos_term, sin_term]),
                ]))
            }
        }
    }
...
mod tests {
    ...
    fn test_simple_harmonic_oscillator() {
        ...
        let solver = ConstantCoeffSecondOrderSolver::new();
        let solution = solver.solve(&expr!(1), &expr!(0), &expr!(4), &expr!(0), &y, &x, None);

        assert!(solution.is_ok(), "Simple harmonic oscillator should solve");
        let sol = solution.unwrap();
        let sol_str = sol.to_string();
        assert!(
            sol_str.contains("cos") || sol_str.contains("sin"),
            "Solution should contain trigonometric functions, got: {}",
            sol_str
        );
    }
    ...
    fn test_distinct_real_roots() {
        ...
        let solver = ConstantCoeffSecondOrderSolver::new();
        let solution = solver.solve(&expr!(1), &expr!(-3), &expr!(2), &expr!(0), &y, &x, None);

        assert!(solution.is_ok(), "Distinct real roots case should solve");
        let sol = solution.unwrap();
        let sol_str = sol.to_string();
        assert!(
            sol_str.contains("exp"),
            "Solution should contain exp, got: {}",
            sol_str
        );
    }
    ...
    fn test_repeated_real_root() {
        ...
        let solver = ConstantCoeffSecondOrderSolver::new();
        let solution = solver.solve(&expr!(1), &expr!(-2), &expr!(1), &expr!(0), &y, &x, None);
    ...
    fn test_complex_with_damping() {
        ...
        let solver = ConstantCoeffSecondOrderSolver::new();
        let solution = solver.solve(&expr!(1), &expr!(2), &expr!(5), &expr!(0), &y, &x, None);
    ...
    fn test_zero_coefficient_a() {
        ...
        let solver = ConstantCoeffSecondOrderSolver::new();
        let solution = solver.solve(&expr!(0), &expr!(1), &expr!(1), &expr!(0), &y, &x, None);
