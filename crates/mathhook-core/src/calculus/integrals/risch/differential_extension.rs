...
fn detect_exponential_extension(expr: &Expression, var: Symbol) -> Option<DifferentialExtension> {
    match expr {
        Expression::Function { name, args } if name == "exp" && args.len() == 1 => {
            ...
            if contains_variable(arg, var.clone()) {
...
fn detect_logarithmic_extension(expr: &Expression, var: Symbol) -> Option<DifferentialExtension> {
    ...
    match expr {
        Expression::Function { name, args } if (name == "ln" || name == "log") && args.len() == 1 => {
            let arg = &args[0];
        ...
        Expression::Mul(_) => {
            ...
            if let Some((num, den)) = extract_division(expr) {
                ...
                if is_one(&num) && contains_variable(&den, var.clone()) {
...
fn derivative_of(expr: &Expression, var: Symbol) -> Expression {
    match expr {
        ...
        Expression::Add(terms) => {
            // Sum rule: (f+g)' = f' + g'
            Expression::add(terms.iter().map(|t| derivative_of(t, var.clone())).collect())
        }
        Expression::Pow(base, exp) => {
            // Power rule for constant exponent
            if !contains_variable(exp, var.clone()) {
                // (x^n)' = n*x^(n-1) * x'
                let base_derivative = derivative_of(base, var);
                Expression::mul(vec![
                    (**exp).clone(),
                    Expression::pow((**base).clone(), Expression::add(vec![(**exp).clone(), Expression::integer(-1)])),
                    base_derivative,
                ])
            } else {
                Expression::integer(0)
...
mod tests {
    ...
    fn test_detect_exponential_simple() {
        ...
        assert!(ext.is_some());
        assert!(matches!(ext.unwrap(), DifferentialExtension::Exponential { .. }));
    }
    ...
    fn test_detect_logarithmic_simple() {
        ...
        assert!(ext.is_some());
        assert!(matches!(ext.unwrap(), DifferentialExtension::Logarithmic { .. }));
    }
    ...
    fn test_contains_variable_false() {
        let x = symbol!(x);
        let y = symbol!(y);
        let expr = Expression::mul(vec![Expression::integer(2), Expression::symbol(y)]);
