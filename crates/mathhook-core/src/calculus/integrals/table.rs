    ...
    match expr {
            ...
            if let Expression::Symbol(s) = &**base {
                if s == var {
                    ...
                    if let Expression::Mul(factors) = &**exp {
                        if factors.len() == 2 {
                            if let (Expression::Number(Number::Integer(1)),
                                   Expression::Pow(two, neg_one)) = (&factors[0], &factors[1]) {
        ...
        Expression::Function { name, args } => {
            ...
            match name.as_str() {
                ...
                "ln" => if coeff == 1 { Some(PatternKey::NaturalLog) } else { None },
                "sin" => Some(PatternKey::Sine { coefficient: coeff }),
                "cos" => Some(PatternKey::Cosine { coefficient: coeff }),
                "tan" => if coeff == 1 { Some(PatternKey::Tangent) } else { None },
                "cot" => if coeff == 1 { Some(PatternKey::Cotangent) } else { None },
                "sec" => if coeff == 1 { Some(PatternKey::Secant) } else { None },
                "csc" => if coeff == 1 { Some(PatternKey::Cosecant) } else { None },
                "sinh" => if coeff == 1 { Some(PatternKey::HyperbolicSine) } else { None },
                "cosh" => if coeff == 1 { Some(PatternKey::HyperbolicCosine) } else { None },
                "tanh" => if coeff == 1 { Some(PatternKey::HyperbolicTangent) } else { None },
                "sqrt" => {
                ...
                if let Expression::Pow(denom, exp) = factor {
                    if matches!(**exp, Expression::Number(Number::Integer(-1))) {
                        ...
                        if let Expression::Add(terms) = &**denom {
                            if terms.len() == 2 {
                                if let (Expression::Pow(x_base, two1), Expression::Number(Number::Integer(a_sq))) =
                                    (&terms[0], &terms[1]) {
                                    if matches!(**two1, Expression::Number(Number::Integer(2))) {
                                        if let Expression::Symbol(s) = &**x_base {
                                            if s == var && *a_sq > 0 {
                                                return Some(PatternKey::ArctanPattern { a_squared: *a_sq });
                                            }
                                        }
                                    }
                                }
                            ...
                            if name == "sqrt" && args.len() == 1 {
                                if let Expression::Add(terms) = &args[0] {
                                    if terms.len() == 2 {
                                        ...
                                        if let (Expression::Number(Number::Integer(a_sq)),
                                               Expression::Mul(neg_x_sq_factors)) = (&terms[0], &terms[1]) {
                                            if *a_sq > 0 && neg_x_sq_factors.len() == 2 {
                                                if let (Expression::Number(Number::Integer(-1)),
                                                    ...
                                                    if matches!(**two, Expression::Number(Number::Integer(2))) {
                                                        if let Expression::Symbol(s) = &**x_base {
                                                            if s == var {
                                                                return Some(PatternKey::ArcsinPattern {
                                                                    a_squared: *a_sq
                                                                });
                                                            }
                                                        }
...
fn integrate_pattern(pattern: &PatternKey, var: &Symbol) -> Expression {
    ...
    match pattern {
        ...
        PatternKey::Tangent => {
            ...
            Expression::mul(vec![
                ...
                Expression::function("ln", vec![
                    Expression::function("abs", vec![Expression::function("cos", vec![x])])
                ]),
            ])
        }
        ...
        PatternKey::Cotangent => {
            // ∫cot(x) dx = ln|sin(x)|
            Expression::function("ln", vec![
                Expression::function("abs", vec![Expression::function("sin", vec![x])])
            ])
        }
        ...
        PatternKey::Secant => {
            ...
            Expression::function("ln", vec![
                Expression::function("abs", vec![Expression::add(vec![sec_x, tan_x])])
            ])
        }
        ...
        PatternKey::Cosecant => {
            ...
            Expression::mul(vec![
                Expression::integer(-1),
                Expression::function("ln", vec![
                    Expression::function("abs", vec![Expression::add(vec![csc_x, cot_x])])
                ]),
            ])
        }
        ...
        PatternKey::ArctanPattern { a_squared } => {
            ...
            Expression::mul(vec![
                Expression::rational(1, a),
                Expression::function("atan", vec![
                    Expression::mul(vec![x, Expression::rational(1, a)])
                ]),
            ])
        }
...
fn is_constant_wrt(expr: &Expression, var: &Symbol) -> bool {
    match expr {
        ...
        Expression::Pow(base, exp) => {
            is_constant_wrt(base, var) && is_constant_wrt(exp, var)
        }
