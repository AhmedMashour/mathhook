...
fn analyze_and_explain(expr: &Expression, var: &Symbol) -> (Vec<String>, String) {
    match expr {
        ...
        Expression::Pow(base, exp) => {
            if matches!(base.as_ref(), Expression::Symbol(s) if s.name() == var.name()) {
                (
                    vec![
                        format!(
                            "Power rule: integral({}^n d{}) = {}^(n+1)/(n+1) + C",
                            var.name(),
                            var.name(),
                            var.name()
                        ),
        ...
        Expression::Mul(factors) => {
            ...
            let has_var = factors
                .iter()
                .any(|f| matches!(f, Expression::Symbol(s) if s.name() == var.name()));
            let has_func = factors
                .iter()
                .any(|f| matches!(f, Expression::Function { .. }));
            ...
            if has_const && has_var && has_func {
                ...
                (
                    vec![
                        format!("Product integration"),
                        format!("Consider integration by parts or substitution"),
                    ],
                    "by_parts".to_owned(),
                )
            }
...
pub fn explain_power_rule(
    ...
    let result = Expression::mul(vec![
        ...
        Expression::pow(exponent_plus_one, Expression::integer(-1)),
    ]);
    ...
    StepByStepExplanation {
        ...
        steps,
        total_steps: 3,
        rules_used: vec!["Power Rule for Integration".to_owned()],
    }
}
...
pub fn explain_u_substitution(
    ...
    StepByStepExplanation {
        ...
        steps,
        total_steps: 6,
        rules_used: vec!["U-Substitution".to_owned()],
    }
}
...
pub fn explain_integration_by_parts(
    ...
    StepByStepExplanation {
        ...
        steps,
        total_steps: 7,
        rules_used: vec!["Integration by Parts".to_owned()],
    }
}
...
pub fn explain_definite_integral(
    ...
    StepByStepExplanation {
        ...
        steps,
        total_steps: 5,
        rules_used: vec!["Fundamental Theorem of Calculus".to_owned()],
    }
}
