...
impl Limits for Expression {
    fn limit(&self, variable: &Symbol, point: &Expression) -> Expression {
        // Check for rational function pattern BEFORE substitution
        // This handles f(x)/g(x) written as f(x) * g(x)^(-n)
        match self {
            Expression::Mul(factors) if factors.len() == 2 => {
                // Check for rational function: numerator * denominator^(negative)
                // This handles x^(-1), x^(-2), etc.
                if let Expression::Pow(denom, exp) = &factors[1] {
                    if let Expression::Number(Number::Integer(n)) = exp.as_ref() {
                        if *n < 0 {
                            // Convert: numer * denom^(-n) → numer / denom^n
                            let positive_exp = Expression::integer(-n);
                            let denominator_positive =
                                Expression::pow(denom.as_ref().clone(), positive_exp);
                            return LimitMethods::rational_limit(
                                &factors[0],
                ...
                if let Expression::Pow(denom, exp) = &factors[0] {
                    if let Expression::Number(Number::Integer(n)) = exp.as_ref() {
                        if *n < 0 {
                            // Convert: denom^(-n) * numer → numer / denom^n
                            let positive_exp = Expression::integer(-n);
                            let denominator_positive =
                                Expression::pow(denom.as_ref().clone(), positive_exp);
                            return LimitMethods::rational_limit(
        ...
        // For other cases, try substitution first
        ...
        match self {
            Expression::Mul(_factors) => LimitMethods::trigonometric_limit(self, variable, point),
    ...
    fn limit_at_infinity(&self, variable: &Symbol) -> Expression {
        match self {
            ...
            Expression::Pow(base, exp) => {
                // (1 + 1/x)^x → e
                if let Expression::Symbol(exp_sym) = exp.as_ref() {
                    if exp_sym == variable {
                        let base_limit = base.limit_at_infinity(variable);
                        if base_limit == Expression::integer(1) {
                            return Expression::e();
                        }
                    }
                }
                ...
                if !denom_factors.is_empty() {
                    ...
                    match (num_degree, den_degree) {
                        ...
                        (None, Some(_)) | (Some(_), None) => {
                            return LimitMethods::rational_limit_at_infinity(
                                &numerator,
                                &denominator,
                                variable,
                            );
