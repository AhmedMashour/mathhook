...
fn to_napi_doc(doc: &str, indent: usize) -> String {
    ...
    for line in &lines[1..lines.len() - 1] {
        result.push_str(&escape_doc_string(line.trim()));
        result.push_str("\\n");
    }
    if let Some(last) = lines.last() {
        result.push_str(&escape_doc_string(last.trim()));
    }
    result.push_str("\"]");
    result
}
