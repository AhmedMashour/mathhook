...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --all)
            ...
            ;;
        -h|--help)
            echo "Usage: $0 [--dry-run] [--crates] [--pypi] [--npm] [--all]"
            echo ""
            echo "Options:"
...
if $PUBLISH_CRATES; then
    ...
    for crate in "${CRATES_ORDER[@]}"; do
        ...
        if $DRY_RUN; then
            cargo publish --dry-run ${CARGO_REGISTRY_TOKEN:+--token "$CARGO_REGISTRY_TOKEN"}
            log_info "[DRY RUN] Would publish $crate"
        else
            # Try to publish, continue if already published
            if cargo publish ${CARGO_REGISTRY_TOKEN:+--token "$CARGO_REGISTRY_TOKEN"} 2>&1; then
...
if $PUBLISH_PYPI; then
    ...
    if $DRY_RUN; then
        log_info "Building wheel (dry run)..."
        maturin build --release
        log_info "[DRY RUN] Would publish to PyPI"
        log_info "Built wheel:"
        ls -la target/wheels/*.whl 2>/dev/null || true
    else
        log_info "Building and publishing to PyPI..."

        # Use maturin publish (handles building and uploading)
        if [ -n "${PYPI_API_TOKEN:-}" ]; then
            # Use token from environment
            MATURIN_PYPI_TOKEN="$PYPI_API_TOKEN" maturin publish --skip-existing
        else
...
if $PUBLISH_NPM; then
    ...
    log_info "Building native binding..."
    npm run build
