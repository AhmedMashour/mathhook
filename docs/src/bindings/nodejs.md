...
const resultCustom = expr.evaluateWithContext(ctxCustom);
```
...
// Numerical context with variables (use array of tuples)
const ctx = EvalContext.numeric([
    ['x', integer(3)],
    ['y', integer(4)]
]);

// Custom context with options
...
import {
    ...
    exp, ln, log10, sqrt,                 // Exponential/Logarithmic
...
// Trigonometric
const expr1 = sin(x).pow(2).add(cos(x).pow(2));
const result1 = simplify(expr1);  // 1

// Inverse trigonometric
const expr2 = asin(sin(x));
const result2 = simplify(expr2);  // x (with domain assumptions)

// Hyperbolic
const expr3 = sinh(x).pow(2).subtract(cosh(x).pow(2));
const result3 = simplify(expr3);  // -1

// Exponential and logarithmic
const expr4 = exp(ln(x));
const result4 = simplify(expr4);  // x

// Square root
const expr5 = sqrt(x.pow(2));
const result5 = simplify(expr5);  // |x| (with assumption handling)

// Rounding and sign functions
const v1 = floor(2.7);  // 2
const v2 = ceil(2.3);   // 3
const v3 = round(2.5);  // 3 (rounds to nearest even)
const v4 = sign(-5);    // -1
const v5 = abs(-5);     // 5
```
...
const x = Symbol.create('x');
const n = Symbol.create('n');

// Gamma function and factorial
const expr1 = gamma(x.add(1)).divide(gamma(x));
const result1 = simplify(expr1);  // x

const expr2 = factorial(5);
const result2 = simplify(expr2);  // 120

// Digamma (logarithmic derivative of gamma)
const expr3 = digamma(1);  // -γ (Euler-Mascheroni constant)

// Riemann zeta function
const expr4 = zeta(2);  // π²/6

// Error functions
const expr5 = erf(0);   // 0
const expr6 = erfc(0);  // 1

// Polygamma function (nth derivative of digamma)
const expr7 = polygamma(1, x);  // Trigamma function

// Bessel functions
const expr8 = bessel_j(0, x);  // Bessel function of first kind
const expr9 = bessel_y(0, x);  // Bessel function of second kind

// Beta function
const expr10 = beta(2, 3);  // Β(2,3) = 1/12
```
...
const result1 = gcd(12, 18);  // 6

// Least common multiple
const result2 = lcm(4, 6);  // 12

// Modulo operation
const result3 = modulo(17, 5);  // 2

// Primality testing
const result4 = isprime(17);  // true
const result5 = isprime(18);  // false

// Symbolic number theory
const x = Symbol.create('x');
const y = Symbol.create('y');
const expr = gcd(x, y);  // Symbolic GCD
```

#### Polynomial Functions

```typescript
import { degree, roots, parse, Symbol } from 'mathhook';

const x = Symbol.create('x');

// Polynomial degree
const poly1 = parse('x^3 + 2*x^2 + x + 1');
const deg = degree(poly1, 'x');  // 3

// Polynomial roots
const poly2 = parse('x^2 - 5*x + 6');
const solutions = roots(poly2, 'x');  // {2, 3}

// Works with symbolic coefficients
const a = Symbol.create('a');
const b = Symbol.create('b');
const c = Symbol.create('c');
const poly3 = a.multiply(x.pow(2)).add(b.multiply(x)).add(c);
const deg2 = degree(poly3, 'x');  // 2
```
...
- [Web Integration Examples](https://github.com/AhmedMashour/mathhook-examples) - Production examples
