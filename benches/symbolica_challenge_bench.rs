...
fn bench_ultimate_performance(c: &mut Criterion) {
    ...
    c.bench_function("compact_expression_performance", |b| {
        b.iter(|| {
            // Test CompactExpression performance directly
            let num1 = CompactNumber::SmallInt(black_box(42));
            let num2 = CompactNumber::SmallInt(black_box(58));
            let expr = Expression::add(vec![
