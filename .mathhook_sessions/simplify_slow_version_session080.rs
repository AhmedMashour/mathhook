...
use crate::core::{CompactNumber, Expression, ExpressionArena, SimdOptimized, Symbol};
...
impl Expression {
        ...
        match self {
            Expression::Add(terms) if terms.len() > 10 => {
                ...
                let simplified_terms: Vec<Expression> =
                    terms.iter().map(|t| t.simplify_with_arena(arena)).collect();
    ...
    /// 🚀 ULTRA-FAST addition simplification - restored performance
    #[inline(always)]
    fn simplify_addition_optimized(&self, terms: &[Expression]) -> Self {
        ...
        // 🚀 ULTRA-FAST PATH: Direct numeric operations without overhead
        ...
        if has_numeric {
            ...
            if total != 0.0 {
                if float_sum != 0.0 || total.fract() != 0.0 {
                    non_numeric.insert(0, Expression::Number(CompactNumber::float(total)));
                } else {
                    non_numeric.insert(0, Expression::integer(total as i64));
                }
            }
        }
    ...
    /// 🚀 ULTRA-FAST PATH: Simple addition without SIMD overhead
    #[inline(always)]
    fn simplify_addition_fast_path(&self, terms: &[Expression]) -> Self {
        let mut int_sum = 0i64;
        let mut float_sum = 0.0f64;
        let mut has_int = false;
        let mut has_float = false;
        let mut non_numeric = Vec::with_capacity(terms.len());
    ...
    fn simplify_two_terms_hot_path(&self, term1: &Expression, term2: &Expression) -> Expression {
        ...
        if let (
            Expression::Number(CompactNumber::SmallInt(n1)),
            Expression::Number(CompactNumber::SmallInt(n2)),
        ) = (term1, term2)
        {
...
mod tests {
    ...
    fn test_performance_benchmark() {
        ...
        println!(
            "Simplification performance: {:.2}M ops/sec",
