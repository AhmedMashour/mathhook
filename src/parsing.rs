        ...
        while i < input.len() {
            ...
            match ch {
                ...
                '/' if paren_depth == 0 => {
                    ...
                    while i < input.len() && input.chars().nth(i).unwrap() != '*' && input.chars().nth(i).unwrap() != '/' {
                            ...
                            if i == 0 {
                                ...
                                match t {
                                    Expression::Mul(factors) if factors.len() > 0 => {
                                        if let Expression::Number(CompactNumber::SmallInt(n)) = &factors[0] {
                                            if *n < 0 {
                                                format!(" - {}", 
                                                    Expression::Mul(Box::new(
                                                        std::iter::once(Expression::Number(CompactNumber::SmallInt(-n)))
            ...
            Expression::Mul(factors) => {
                if factors.is_empty() {
                    ...
                    let factor_strs: Vec<String> = factors.iter()
                        .map(|f| {
                            match f {
                                Expression::Add(_) => format!("\\left({}\\right)", f.to_latex_advanced()),
