...
impl Expression {
    ...
    fn simplify_power_ultra_fast(&self, base: &Expression, exp: &Expression) -> Self {
        match (base, exp) {
            ...
            // 0^n = 0 (for n > 0)
            (Expression::Number(Number::SmallInt(0)), Expression::Number(Number::SmallInt(n))) if *n > 0 => {
                Expression::integer(0)
            },
            // 1^n = 1
            (Expression::Number(Number::SmallInt(1)), _) => Expression::integer(1),
            // Direct numeric powers for small integers
            (Expression::Number(Number::SmallInt(base_val)), Expression::Number(Number::SmallInt(exp_val))) => {
