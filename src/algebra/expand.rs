    ...
    fn expand(&self) -> Self {
        match self {
            ...
            Expression::Add(terms) => {
                ...
                let expanded_terms: Vec<Expression> = terms.iter()
                    .map(|term| term.expand())
                    .collect();
                Expression::add(expanded_terms)
            },
            
            Expression::Mul(factors) => {
                self.expand_multiplication(factors)
            },
            
            Expression::Pow(base, exp) => {
                self.expand_power(base, exp)
            },
            
            Expression::Function { name, args } => {
                // Expand arguments
                let expanded_args: Vec<Expression> = args.iter()
                    .map(|arg| arg.expand())
                    .collect();
    ...
    fn distribute_multiply(&self, left: &Expression, right: &Expression) -> Expression {
        match (left, right) {
            ...
            (Expression::Add(left_terms), _) => {
                let distributed_terms: Vec<Expression> = left_terms.iter()
            ...
            (_, Expression::Add(right_terms)) => {
                let distributed_terms: Vec<Expression> = right_terms.iter()
    ...
    fn expand_power(&self, base: &Expression, exp: &Expression) -> Expression {
        ...
        if let Expression::Number(Number::Integer(n)) = exp {
            if let Some(exp_val) = n.to_i64() {
                if exp_val >= 0 && exp_val <= 10 { // Reasonable limit
        ...
        Expression::pow(base.expand(), exp.expand())
    }
