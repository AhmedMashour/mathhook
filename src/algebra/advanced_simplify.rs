...
impl AdvancedSimplify for Expression {
        ...
        match self {
            ...
            Expression::Add(terms) => {
                let simplified_terms: Vec<Expression> = terms.iter()
                    .map(|term| term.simplify_factorial())
                    .collect();
                Expression::add(simplified_terms)
            },
            
            Expression::Mul(factors) => {
                let simplified_factors: Vec<Expression> = factors.iter()
            ...
            Expression::Function { name, args } => {
                let simplified_args: Vec<Expression> = args.iter()
                    .map(|arg| arg.simplify_factorial())
                    .collect();
    ...
    fn simplify_logarithms(&self) -> Self {
        match self {
            ...
            Expression::Function { name, args } if name == "ln" => {
...
impl Expression {
        ...
        match arg {
            Expression::Number(Number::Integer(n)) => {
                if let Some(val) = n.to_i64() {
                    if val >= 0 && val <= 20 { // Reasonable limit to prevent overflow
            ...
            Expression::Add(terms) if terms.len() == 2 => {
                ...
                if let (Expression::Symbol(s), Expression::Number(Number::Integer(offset))) = (&terms[0], &terms[1]) {
                    if offset == &BigInt::from(-1) {
    ...
    fn simplify_log_function(&self, args: &[Expression]) -> Expression {
        if args.len() == 1 {
            match &args[0] {
                Expression::Number(Number::Integer(n)) if n.is_one() => {
                    // log(1) = 0
                    Expression::integer(0)
                },
                
                Expression::Number(Number::Integer(n)) if n == &BigInt::from(10) => {
                ...
                // log(a*b) = log(a) + log(b)
                Expression::Mul(factors) => {
                    let log_terms: Vec<Expression> = factors.iter()
            ...
            match &args[0] {
                Expression::Number(Number::Integer(n)) => {
                    if n.is_zero() {
                        ...
                        if let Some(sqrt_val) = self.integer_sqrt(n) {
