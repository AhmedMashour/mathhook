    ...
    fn factor(&self) -> Self {
        match self {
            ...
            Expression::Add(terms) => {
                self.factor_addition(terms)
            },
            
            Expression::Mul(factors) => {
                // Factor each factor and recombine
                let factored_factors: Vec<Expression> = factors.iter()
                    .map(|f| f.factor())
                    .collect();
            ...
            Expression::Pow(base, exp) => {
                Expression::pow(base.factor(), exp.factor())
            },
            
            Expression::Function { name, args } => {
                let factored_args: Vec<Expression> = args.iter()
                    .map(|arg| arg.factor())
                    .collect();
    ...
    fn factor_out_gcd(&self) -> Self {
        match self {
            Expression::Add(terms) => {
                ...
                if !common_factor.is_one() {
                    let factored_terms: Vec<Expression> = terms.iter()
                        .map(|term| self.divide_by_factor(term, &common_factor))
                        .collect();
                    
                    Expression::mul(vec![
                        common_factor,
                        Expression::add(factored_terms)
                    ])
                } else {
                    self.clone()
                }
...
impl Expression {
    ...
    fn intersect_factors(&self, factors1: &[Expression], factors2: &[Expression]) -> Vec<Expression> {
    ...
    fn divide_by_factor(&self, expr: &Expression, factor: &Expression) -> Expression {
        match (expr, factor) {
            ...
            // Symbol division
            (Expression::Symbol(s1), Expression::Symbol(s2)) if s1 == s2 => {
                Expression::integer(1)
            },
    ...
    fn try_quadratic_factoring(&self, terms: &[Expression]) -> Option<Expression> {
    ...
    pub fn factor_difference_of_squares(&self, a: &Expression, b: &Expression) -> Expression {
        Expression::mul(vec![
            ...
            Expression::add(vec![a.clone(), Expression::mul(vec![Expression::integer(-1), b.clone()])])
        ])
    }
