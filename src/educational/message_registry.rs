...
impl MessageKey {
    pub const fn new(category: MessageCategory, message_type: MessageType, variant: u8) -> Self {
        Self { category, message_type, variant }
    }
}
...
impl MessageTemplate {
    pub const fn new(title: &'static str, emoji: &'static str, content: &'static str, placeholders: &'static [&'static str]) -> Self {
        Self { title, emoji, content, placeholders }
    }
}
...
impl MessageBuilder {
    ...
    pub fn build(self) -> Option<crate::core::step_by_step::Step> {
        if let Some(template) = MESSAGE_REGISTRY.get(&self.key) {
            ...
            Some(crate::core::step_by_step::Step::new(title, content))
        } else {
            None
        }
    }
...
impl EducationalMessageGenerator {
    ...
    pub fn linear_equation_steps(equation: &str, variable: &str, solution: &str) -> Vec<crate::core::step_by_step::Step> {
        vec![
            MessageBuilder::new(MessageCategory::LinearEquation, MessageType::Introduction, 0)
            ...
            MessageBuilder::new(MessageCategory::LinearEquation, MessageType::Verification, 0)
                .with_substitution("variable", variable)
                .with_substitution("solution", solution)
                .with_substitution("verification", &format!("{}({}) + constant", variable, solution))
                .build()
                .unwrap(),
        ]
    }
    ...
    pub fn quadratic_equation_steps(equation: &str, variable: &str, a: &str, b: &str, c: &str, solutions: &str) -> Vec<crate::core::step_by_step::Step> {
        vec![
            MessageBuilder::new(MessageCategory::QuadraticEquation, MessageType::Introduction, 0)
                .with_substitution("equation", equation)
                .with_substitution("variable", variable)
                .build()
                .unwrap(),
    ...
    pub fn error_explanation(category: MessageCategory, error_type: u8, context: &HashMap<String, String>) -> Option<crate::core::step_by_step::Step> {
    ...
    pub fn mathematical_insight(variant: u8, variable: &str) -> Option<crate::core::step_by_step::Step> {
...
impl MessageOptimizer {
    ...
    pub fn precompute_common_messages() -> HashMap<String, Vec<crate::core::step_by_step::Step>> {
        ...
        cache.insert(
            ...
            EducationalMessageGenerator::quadratic_equation_steps("x² - 4", "x", "1", "0", "-4", "x = ±2")
        );
    ...
    pub fn get_optimized_message(scenario: &str) -> Option<Vec<crate::core::step_by_step::Step>> {
        static CACHE: Lazy<HashMap<String, Vec<crate::core::step_by_step::Step>>> = 
...
mod tests {
    ...
    fn test_message_builder() {
        let step = MessageBuilder::new(MessageCategory::LinearEquation, MessageType::Introduction, 0)
    ...
    fn test_message_performance() {
        ...
        // Generate 100,000 messages using registry
        for i in 0..100_000 {
            let variant = (i % 2) as u8;
            let _step = MessageBuilder::new(MessageCategory::LinearEquation, MessageType::Introduction, variant)
                .with_substitution("equation", "test")
                .with_substitution("variable", "x")
                .build();
        }
        ...
        println!("Message registry performance: {:.2}M messages/sec", messages_per_sec / 1_000_000.0);
        
        // Should be fast with registry lookup
        assert!(messages_per_sec >= 500_000.0, 
               "Registry should be fast, got {:.2}M/sec", 
