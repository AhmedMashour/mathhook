...
fn test_symbolica_arithmetic_domination() {
    ...
    println!("Arithmetic domination: {:.2}M ops/sec", ops_per_sec / 1_000_000.0);
...
fn test_symbolica_power_domination() {
    ...
    println!("Power domination: {:.2}M ops/sec", ops_per_sec / 1_000_000.0);
...
fn test_symbolica_bulk_operations_domination() {
    ...
    let simplified_expressions: Vec<Expression> = expressions
        .into_iter()
        .map(|e| e.simplify())
        .collect();
    ...
    println!("Bulk operations domination: {:.2}K ops/sec", ops_per_sec / 1_000.0);
