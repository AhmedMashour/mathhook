//! 🎯 MAGIC BULLET #2 VERIFICATION: Expression IS CompactExpression (32-byte optimized)

use mathhook::core::{Expression, Symbol, CompactNumber};
...
#[test]
fn test_compact_number_integration() {
    // Test CompactNumber integration
    let small_int = Expression::integer(42);
    let big_int = Expression::integer(i64::MAX as i128 + 1);
    let float_val = Expression::from(3.14);
    
    // Verify they're all Expression::Number variants
    match small_int {
        Expression::Number(CompactNumber::SmallInt(n)) => assert_eq!(n, 42),
        _ => panic!("Small integer should use SmallInt variant"),
    }
    
    match big_int {
        Expression::Number(CompactNumber::BigInteger(_)) => {}, // Expected for large values
        Expression::Number(CompactNumber::SmallInt(_)) => {}, // Also OK if it fits
        _ => panic!("Big integer should use appropriate CompactNumber variant"),
    }
    
    match float_val {
        Expression::Number(CompactNumber::Float(f)) => assert!((f - 3.14).abs() < 1e-10),
        _ => panic!("Float should use Float variant"),
    }
    
    println!("✅ Magic Bullet #2: CompactNumber integration verified!");
}
...
fn test_performance_optimized_constructors() {
    ...
    assert!(empty_add.is_zero());
    
    let single_add = Expression::add(vec![Expression::integer(5)]);
    match single_add {
        Expression::Number(CompactNumber::SmallInt(5)) => {},
    ...
    let single_mul = Expression::mul(vec![Expression::integer(7)]);
    match single_mul {
        Expression::Number(CompactNumber::SmallInt(7)) => {},
